...
public class ApiObserver extends ApiStorage implements IApiObserver {
    ...
    @Override
    public <Item extends RealmObject> Observable<Item> getApiObservableSingle(Observable<Item> api, Class<Item> realmClass, String idField, String id) {
        if (getStorage() != null) {
            Item result = getItem(realmClass, idField, id);
            Observable<Item> realmObserver = Observable.just(result);
            Observable<Item> retrofitObserver = api
                    .compose(applySchedulers())
                    .compose(getLifecycle());
            return Observable.<Item>create(subscriber -> {
                realmObserver.take(2).subscribe(subscriber::onNext, subscriber::onError, subscriber::onCompleted);
                retrofitObserver.subscribe(this::setItem, subscriber::onError);
            }).compose(getLifecycle());
        } else
            return api;
    }
